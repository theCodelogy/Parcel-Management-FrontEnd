...
interface AuthState {
  ...
  salary: number;
  status: string;
  // hub field is no longer needed in the form data since it will be set automatically
...
const CreateDeliveryManPage: React.FC = () => {
  ...
  const onSubmit = async (data: AuthState): Promise<void> => {
    ...
    const payload: TDeliveryMan = {
      ...
      // Set hub to the current user's name instead of using form data
      hub: name,
      status: "Active",
      ...
      <div className="w-full bg-white rounded-2xl shadow-xl overflow-auto p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* The hub select has been removed.
                The hub field will now directly use the current user's name. */}
