  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (filters.trackerId) params.append("TrakingId", filters.trackerId);
        if (filters.recipient) params.append("pickupPoints", filters.recipient);
        if (filters.merchant) params.append("merchant", filters.merchant);
        if (filters.date) params.append("pickupDate", filters.date);
        if (filters.status) params.append("currentStatus", filters.status);

        const response = await axios.get<{ data: ApiResponse[] }>(
          `https://parcel-management-back-end-peach.vercel.app/api/v1/parcel?${params.toString()}`
        );
        const trackers = response.data.data.map((item: ApiResponse) => ({
          id: item._id,
          trackingId: item.TrakingId,
