...
export const generateStatus = async <T extends Record<string, unknown>>(
  ...
  if (payload.title == "Parcel Create") {
    ...
    const res = await fetch(
      `https://parcel-management-back-end.vercel.app/api/v1/deliveryMan?name=${payload.statusDetails.pickupMan}`,
      {
        method: "GET",
      }
    );
    ...
    const res = await fetch(
      `http://localhost:5000/api/v1/deliveryMan?name=${payload.statusDetails.deliveryMan}`,
      {
        method: "GET",
      }
    );
